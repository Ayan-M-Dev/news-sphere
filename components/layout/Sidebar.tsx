...
export default function Sidebar() {
    ...
    return (
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-background border-t border-border md:hidden">
        <div className="flex items-center justify-around h-16 px-2">
          ...
          <button
            onClick={logout}
            className="flex flex-col items-center justify-center gap-1 px-2 py-1 rounded-lg transition-colors flex-1 min-w-0 text-muted-foreground hover:text-foreground"
            aria-label="Logout"
          >
            <svg
              className="w-5 h-5"
  ...
  return (
    <>
      <aside
        className={`fixed left-0 top-0 h-full bg-background border-r border-border transition-[width] duration-300 ease-in-out z-40 overflow-hidden ${
          ...
          <div className="flex items-center p-4 border-b border-border shrink-0">
            {!isCollapsed && (
              <div className="flex items-center gap-2 whitespace-nowrap">
                ...
                />
                <span className="font-bold overflow-hidden">News Sphere</span>
              </div>
            )}
            {isCollapsed && (
              <Image
                ...
                width={32}
                height={32}
                className="h-8 w-8 object-contain mx-auto shrink-0"
              />
            )}
          </div>
